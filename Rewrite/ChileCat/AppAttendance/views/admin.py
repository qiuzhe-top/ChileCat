...
class TaskRestKnowing(TaskBase):
    ...
    def get_context(self, request, *args, **kwargs):
        task = self.get_task_by_user()
        RoomHistory.objects.filter(task=task).update(
            is_knowing=False)  # 所有寝室为未检查
        TaskFloorStudent.objects.filter(task=task).update(
            flg=True)  # 所有学生为在寝
...
class TaskRestHealth(TaskBase):
    ...
    def get_context(self, request, *args, **kwargs):
        task = self.get_task_by_user()
        # RoomHistory.objects.filter(task=task).update(is_health=False)
...
class a(CoolBFFAPIView):
    ...
    def get_context(self, request, *args, **kwargs):
        ...
        async_to_sync(channel_layer.group_send)('room', {"type": "send.status", "data": {"type": "all_status", "data": 1}})
