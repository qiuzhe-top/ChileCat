...
# LastEditors:  
# LastEditTime: 2022-02-07 10:32:21
...
# from Apps.SchoolAttendance.common.pagination import RecordQueryrPagination
...
#         tasks = models.Task.objects.filter(types=request.params.type)
#         task_list = []
#         for task in tasks:
#             admin_list = task.get_admin()
#             if request.user.username in admin_list:
#                 task_list.append(task)
#         return serializers.TaskObtain(task_list, many=True,
...
#             is_knowing=False)  # 所有寝室为未检查
#         models.TaskFloorStudent.objects.filter(task=task).update(
#             flg=True)  # 所有学生为在寝
...
#         db_username_list, flg = self.qrpc(
#             'GetUserManage',
#             {
...
#         records = models.Record.objects.filter(q1, q2, q3, q4)
...
#     def get_context(self, request, *args, **kwargs):
#         user = request.user
#         task = models.Task.objects.get_or_create(
...
#         # 获取所有历史早签记录
#         query = models.Record.objects.filter(task=task, rule=rule).values(
...
#         models.Record.objects.bulk_create(wait_create_record)
